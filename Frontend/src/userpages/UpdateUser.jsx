...
function UpdateUser() {
  ...
  const fetchUserDataById = async (userId) => {
    try {
      ...
      const response = await UserAccountService.getUserById(userId, token); // Pass userId to getUserById
      const { name, email, role} = response.userAccount;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (confirmDelete) {
        const token = localStorage.getItem('token');
        const res = await UserAccountService.updateUser(userId, userData, token);
        console.log(res)
