...
function UserManagementPage() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await UserAccountService.fetchUsers(token);
      //   console.log(response);
      setUsers(response.userAccounts); // Assuming the list of users is under the key 'userAccounts'
    } catch (error) {
  ...
  const deleteUser = async (userId) => {
    try {
      ...
      const token = localStorage.getItem('token'); // Retrieve the token from localStorage
      if (confirmDelete) {
        await UserAccountService.deleteUser(userId, token);
